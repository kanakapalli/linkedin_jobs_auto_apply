...
class EasyApplyLinkedin:
    ...
    def job_search(self):
        ...
        print("job_search start")
        # go to Jobs
        # jobs_link = self.driver.find_element_by_link_text('Jobs')
        # jobs_link.click()
        self.driver.get("https://www.linkedin.com/jobs/?showJobAlertsModal=false")
        # search based on keywords and location and hit enter 

        search_keywords = self.driver.find_element_by_id('jobs-search-box-keyword-id-ember18')
        search_keywords.clear()
        search_keywords.send_keys(self.keywords)
        search_location = self.driver.find_element_by_id("jobs-search-box-location-id-ember18")
    ...
    def filter(self):
        ...
        time.sleep(1)
        job_type_intern = self.driver.find_element_by_xpath("//label[@for='jobType-I']")
        job_type_intern.click()
        time.sleep(1)
        apply_filter_button = self.driver.find_element_by_xpath("//button[@data-control-name='all_filters_apply']")
        apply_filter_button.click()

    def find_offers(self):
        print("find_offers started")
